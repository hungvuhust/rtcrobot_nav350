...
sys.path.insert(
    1, '/home/btu/ros2_ws/src/rtcrobot_interfaces/rtcrobot_interfaces/')
...
Cmd_setCurrentPose = 'mNPOSSetPose'
...
class Publisher_(Node):
        ...
        try:
            ...
            if (self.current_reflector != nav_350.reflector):
                if (nav_350.reflector < 3):
                    log.loginfo("[NAV350] Miss Reflector %s",
                                nav_350.reflector)
    ...
    def broadcast_tf(self):
        try:
            ...
            self.trans.transform.translation.z = 2.6
            ...
            self.trans_odom.transform.translation.z = 0.0
            # if self.pos_theta >= 0 and self.pos_theta <= math.pi:
            #     self.pos_theta = self.pos_theta - math.pi
            # elif self.pos_theta < 0 and self.pos_theta >= -math.pi:
            #     self.pos_theta = math.pi + self.pos_theta
...
#         self.switch_map = self.create_service(SwitchMap, "switch_map", self.switch_map_callback)
#     def switch_map_callback(self, request, response):
#         log.loginfo('[NAV350] Request: '+str(request.map_layer))
#         response.status = 1
#         response.error_code = 0
#         log.loginfo('Response: '+str(response.error_code))
#         return response
...
class NAV350(Node):
    ...
    def __init__(self):
        ...
        self.nav_address = '192.168.5.98'
        self.nav_port = 2111
        self.nav350 = Nav350Data()
        self.nav350_get_all = self.create_publisher(
            Reflector, "all_reflector", 10)
        self.nav350_get_current = self.create_publisher(
            Reflector, "current_reflector", 10)
        self.nav350_mode = self.create_subscription(
            Nav350Mode, "nav350_mode", self.mode_callback, 10)

        self.nav350_add = self.create_subscription(
            AddLandmark, "add_landmark", self.add_callback, 10)
        self.nav350_delete = self.create_subscription(
            UInt8, "delete_landmark", self.delete_callback, 10)
        self.nav350_edit = self.create_subscription(
            EditLandmark, "edit_landmark", self.edit_callback, 10)
        ...
        log.loginfo('[NAV350] Ver2.0')
        log.loginfo("NAV350 Starting")
        log.loginfo('[NAV350] Spin started')
        self.switch_map = self.create_service(
            SwitchMap, "rtcservice/switch_map", self.switch_map)
    ...
    def switch_map(self, request, response):
        log.loginfo('[NAV350] Request: '+str(request.map_layer))

        self.nav_layer = request.map_layer
        self.flag = 4
        # with mutex:
        #     self.nav350_setCurrentLayer(request.map_layer)
        time.sleep(0.5)
        if (self.current_layer == request.map_layer):
            with mutex:
                setpose = self.nav350_setCurrentPose(
                    request.x, request.y, request.phi)
            if (setpose == 1):
                response.status = 1
                response.error_code = 0
            else:
    ...
    def spin(self):
        while rclpy.ok():
            # time.sleep(0.1)
            a1 = time.time()
            # log.loginfo("[NAV350] SPIN timer")
            self.result_socket = self.socketConnect(self.sock)
            # print(self.flag)
            # print(self.sub_flag)
            with mutex:
                if (self.result_socket == 1):
                    # log.loginfo("[NAV350] Socket Connect OK")

                    if (self.flag_record == 0):
                        self.all_reflector = self.get_all_landmark_in_layer(
                            current_layer)
                    ...
                    if (self.flag == 4 or self.flag == 0):
                        ...
                        self.result_setMode = self.nav350_setMode(
                            Mode_standby)
                        time.sleep(0.1)
                        self.result_setMode = self.nav350_setMode(
                            Mode_navigation)
                        ...
                        self.current_layer = self.nav350_getCurrentLayer()
                        self.setRefsize(28)
                        ...
                        if (self.sub_flag == 1):
                            self.add_landmark(self.add_landmark_.x, self.add_landmark_.y, self.add_landmark_.landmark_type,
                                              self.add_landmark_.reflector_type, self.add_landmark_.size, self.add_landmark_.layer)
                            self.save_permanent()
                            self.flag = 5
                        if self.sub_flag == 2:
                            ...
                            self.flag = 5
                        if (self.sub_flag == 3):
                            self.edit_landmark(self.edit_landmark_.global_id, self.edit_landmark_.x, self.edit_landmark_.y,
                                               self.edit_landmark_.landmark_type, self.edit_landmark_.reflector_type, self.edit_landmark_.size, self.edit_landmark_.layer)
                            self.save_permanent()
                            self.flag = 5
                ...
                if (self.count_publishpose == 50):
                    log.loginfo("[NAV350] Get Data OK %s",
                                self.count_publishpose)
    ...
    def insert_reflector_data(self, global_id, current_layer, x, y, theta):
        try:
            ...
            if self.connection:
                ...
                self.cursor.execute("UPDATE public.reflector SET current_layer = " + (current_layer)+", x = " + str(x) + ", y = " + str(
                    y) + ", theta = " + str(theta) + " WHERE global_id = " + str(self.convert_pos_data(global_id)) + ";")
    ...
    def socketConnect(self, sock=None):
        fist_connect = False
        if (sock == None):
            log.loginfo("[NAV350] First Connect")
            fist_connect = True
        if sock is None:
            try:
                ...
                log.loginfo('[NAV350] Error socket connect %s',
                            traceback.format_exc())
            except Exception as exx:
                log.loginfo('[NAV350] Init socket error! %s',
                            traceback.format_exc())
                self.publish_error(enum.Nav350_Error.ConnectSocket_Fail.value)
        else:
            pass
            ...
            # return 0
        if (self.is_socket_alive(self.sock) == 0 or fist_connect == 1):
            if (fist_connect != 1):
                self.sock.close()
            try:
                log.loginfo("[NAV350] Try Connect")
                ...
                if (self.count_to_error_connect > Error_mark):
                    ...
                    self.publish_error(
                        enum.Nav350_Error.ConnectSocket_TimeOut.value)
                    log.loginfo('[NAV350] Connect Timeout: %s', exx)
                else:
                    log.loginfo('[NAV350] Count to connect error: %s',
                                self.count_to_error_connect)
                return 0
            except Exception as exx:
                self.sock = None
                time.sleep(1)
    ...
    def socketSend(self, msg):
        try:
            ...
            if self.count_nav > 10:
                ...
                self.publish_error(
                    enum.Nav350_Error.SocketSend_Exception.value)
    ...
    def getSlidingMean(self):
        ...
        while i < 11:
            try:
                self.data_getSlidingMean = self.s_socketSend(
                    Code_start + Code_read + Code_space + Cmd_slidingMean + Code_stop)
    ...
    def get_all_landmark_in_layer(self, layer):
        ...
        while i < 11:
            try:
                self.gloabal_ID = []
                self.get_all_reflector = self.s_socketSend(
                    Code_start + Code_method + Code_space + Cmd_read_all_global_ID + Code_space + layer + Code_stop)
                if (self.get_all_reflector != None and self.socket_error_flag == 0):
                    ...
                    if (self.get_all_reflector[2] == 1):
                        ...
                        self.publish_error(
                            enum.Nav350_Error.Invalid_Mode.value)
                    if (self.get_all_reflector[2] == 3):
                        log.loginfo('[NAV350] get all landmark in layer fail')
                        self.publish_error(
                            enum.Nav350_Error.Invalid_Data.value)
                    if (self.get_all_reflector[2] == 7):
                        log.loginfo('[NAV350] get all landmark in layer fail')
                        self.publish_error(
                            enum.Nav350_Error.Genere_Error.value)
                ...
                if (i == 10):
                    log.loginfo(
                        '[NAV350] get all landmark in layer fail: %s', exx)
                    self.publish_error(enum.Nav350_Error.SetMode_Fail.value)
                    return []
    ...
    def get_pose_landmark(self, global_id):
        ...
        while i < 11:
            try:
                self.get_pose_reflector = self.s_socketSend(
                    Code_start + Code_method + Code_space + Cmd_pose_landmark + Code_space + '1' + Code_space + global_id + Code_stop)
                ...
                self.insert_reflector_data(global_id, current_layer, self.convert_pos_data(
                    self.get_pose_reflector[5])/1000, self.convert_pos_data(self.get_pose_reflector[6])/1000, self.convert_theta(self.get_pose_reflector[7]))
                if (i == 10):
    ...
    def add_landmark(self, x, y, landmark_type, reflector_type, size_of_reflector, layer_ID):
        ...
        while i < 11:
            try:
                log.loginfo('[NAV350] add landmark x:%s, y:%s, landmark_type: %s, reflector_type: %s, size_of_reflector: %s, layer: %s', x, y,
                            landmark_type, (reflector_type), str(self.decimal_to_hexadecimal(size_of_reflector)), str(self.decimal_to_hexadecimal(layer_ID)))
                self.add_reflector = self.s_socketSend(Code_start + Code_method + Code_space + Cmd_add_landmark + Code_space + '1' + Code_space + str(x) + Code_space + str(y) + Code_space + str(
                    landmark_type) + Code_space + str(reflector_type) + Code_space + str(self.decimal_to_hexadecimal(size_of_reflector)) + Code_space + '1' + Code_space + str(self.decimal_to_hexadecimal(layer_ID)) + Code_stop)
    ...
    def edit_landmark(self, ID, x, y, landmark_type, reflector_type, size_of_reflector, layer_ID):
        ...
        while i < 11:
            try:
                log.loginfo('[NAV350] add landmark ID:%s, x:%s, y:%s, landmark_type: %s, reflector_type: %s, size_of_reflector: %s, layer: %s', str(self.decimal_to_hexadecimal(
                    ID)), x, y, landmark_type, (reflector_type), (str(self.decimal_to_hexadecimal(size_of_reflector))), str(self.decimal_to_hexadecimal(size_of_reflector)))

                self.edit_reflector = self.s_socketSend(Code_start + Code_method + Code_space + Cmd_edit_landmark + Code_space + '1' + Code_space + str(self.decimal_to_hexadecimal(ID)) + Code_space + str(x) + Code_space + str(
                    y) + Code_space + str(landmark_type) + Code_space + str(reflector_type) + Code_space + str(self.decimal_to_hexadecimal(size_of_reflector)) + Code_space + '1' + Code_space + str(self.decimal_to_hexadecimal(layer_ID)) + Code_stop)
                # Number of landmarks     #x                 #y            #Landmark type     #reflector type  #size of reflector  #layer IDs        #Layer ID
    ...
    def delete_landmark(self, ID):
        ...
        while i < 11:
            try:
                self.delete_reflector = self.s_socketSend(
                    Code_start + Code_method + Code_space + Cmd_delete_landmark + Code_space + '1' + Code_space + str(self.decimal_to_hexadecimal(ID)) + Code_stop)
                # Number of landmarks     #x                 #y            #Landmark type     #reflector type  #size of reflector  #layer IDs        #Layer ID
    ...
    def save_permanent(self):
        ...
        while i < 11:
            try:
                self.store_layout = self.s_socketSend(
                    Code_start + Code_method + Code_space + Cmd_store_layout + Code_stop)
    ...
    def setSlidingMean(self, slidingMean):
        ...
        while i < 11:
            try:
                ...
                self.data_setSlidingMean = self.s_socketSend(
                    Code_start + Code_write + Code_space + Cmd_slidingMean + Code_space + str(slidingMean) + Code_stop)
    ...
    def getRefsize(self):
        ...
        while i < 11:
            try:
                self.data_getRefsize = self.s_socketSend(
                    Code_start + Code_read + Code_space + Cmd_Refsize + Code_stop)
    ...
    def setRefsize(self, refsize):
        ...
        while i < 11:
            try:
                # refsize = hex(refsize)
                # log.loginfo('[nav350] convert refsize:', str(refsize))
                self.data_setRefsize = self.s_socketSend(
                    Code_start + Code_write + Code_space + Cmd_Refsize + Code_space + str(refsize) + Code_stop)
    ...
    def nav350_setMode(self, mode):
        ...
        while i < 11:
            try:
                self.data_setMode = self.socketSend(
                    Code_start + Code_method + Code_space + Cmd_setmode + Code_space + mode + Code_stop)
    ...
    def nav350_setPoseDataFormat(self):
        try:
            data_setPoseDataFormat = self.s_socketSend(
                Code_start + 'sWN' + Code_space + 'NPOSPoseDataFormat' + Code_space + '1' + Code_space + '1' + Code_stop)
    ...
    def nav350_setUserLevel(self):
        ...
        while i < 11:
            try:
                data_setUserLevel = self.s_socketSend(
                    Code_start + Code_method + Code_space + 'SetAccessMode' + Code_space + '03' + Code_space + 'F4724744' + Code_stop)
                ...
                if (i == 10):
                    ...
                    self.publish_error(
                        enum.Nav350_Error.SetUserLevel_Fail.value)
            except Exception as exx:
                i += 1
                if (i == 10):
    ...
    def nav350_setCurrentPose(self, x, y, phi):
        i = 0
        while i < 11:
            try:
                x_send = f"{'-' if x<0 else '+'}{abs(x)}"
                y_send = f"{'-' if x<0 else '+'}{abs(y)}"
                phi_send = f"{'-' if x<0 else '+'}{abs(phi)}"

                data_setCurrentPose = self.s_socketSend(
                    Code_start + Code_method + Code_space + Cmd_setCurrentPose + Code_space + x_send + Code_space + y_send + Code_space + phi_send + Code_stop)
                if (data_setCurrentPose != None and self.socket_error_flag == 0):
                    data_setCurrentPose = data_setCurrentPose.split()
                log.loginfo('[NAV350] set Current Pose ' +
                            str(data_setCurrentPose))

                if self.convert_pos_data(data_setCurrentPose[2]) == 0:
                    # self.nav350_setPoseDataFormat()
                    return 1
                else:
                    i += 1
                if (i == 10):
                    log.loginfo('[NAV350] set Current Pose fail')
                    self.publish_error(
                        enum.Nav350_Error.SetUserLevel_Fail.value)
            except Exception as exx:
                i += 1
                if (i == 10):
                    log.loginfo('[NAV350] set Current Pose fail: %s', exx)
                    self.publish_error(
                        enum.Nav350_Error.SetUserLevel_Fail.value)
                    return 0
        return 0
    ...
    def nav350_setCurrentLayer(self, layer):
        ...
        while i < 11:
            try:
                data_setLayer = self.s_socketSend(
                    Code_start + Code_write + Code_space + Cmd_Layer + Code_space + str(layer) + Code_stop)
    ...
    def nav350_getCurrentLayer(self):
        ...
        while i < 11:
            try:
                data_getLayer = self.s_socketSend(
                    Code_start + Code_read + Code_space + Cmd_Layer + Code_stop)
                ...
                if (data_getLayer != None and self.socket_error_flag == 0):
                    ...
                    if len(data_getLayer) >= 3:
                        ...
                        return self.convert_pos_data(data_getLayer[2])
                    else:
                        i += 1
                else:
                    i += 1
                if (i == 10):
                    ...
                    self.publish_error(
                        enum.Nav350_Error.GetLayer_Exception.value)
            except Exception as exx:
                i += 1
                if (i == 10):
    ...
    def nav350_getPoseData(self):
        ...
        while i < 11:
            try:
                data_getPose = self.socketSend(
                    Code_start + Code_method + Code_space + Cmd_getdata + Code_space + '1' + Code_space + '0' + Code_stop)
                # print(data_getPose)
                # print("-------------------------------------------")
                if (data_getPose != None and self.socket_error_flag == 0):
                    ...
                    if len(data_getPose) >= 16:
                        if (self.count_publishpose > 10 and data_getPose[9] != '0\x03'):
                            pose_x = self.convert_pos_data(
                                data_getPose[7])/1000
                            # print("DKM", pose_x)
                            pose_y = self.convert_pos_data(
                                data_getPose[8])/1000
                            pose_theta = self.convert_theta(data_getPose[9])
                            pos_mode = Position_Mode[self.convert_pos_data(
                                data_getPose[14])]
                            count_reflector = self.convert_pos_data(
                                data_getPose[16])
