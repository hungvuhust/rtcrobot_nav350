...
Cmd_setUserLevel = 'SetAccessMode'
Cmd_setmode = 'mNEVAChangeState'
Cmd_Layer   = 'NEVACurrLayer'
Cmd_Refsize = 'NLMDReflSize'
Cmd_Reftype = 'NLMDReflType'
Cmd_Ncloset = 'NLMDnClosest'
Cmd_Reflector_threshold = 'NLMDReflThreshold'
Cmd_ConfigMapping = 'NMAPMapCfg'
Cmd_doMapping = 'mNMAPDoMapping'
...
class NAV350(Node):
    ...
    def __init__(self):
        ...
        self.nav_address = '192.168.5.198'
        self.nav_port = 2111
        ...
        self.nav350_switch_mode = self.create_subscription(UInt8,"nav350_switch_mode", self.switch_mode_callback, 10)
        ...
        self.create_timer(0.1, self.test_spin)
        self.create_publisher
    def config_reflector(self):
        #using for current reflector: cyclin, 40 mm
        self.setRefType(2)
        self.setRefsize(40)
        self.setRefNClosest(0)
        self.setRefThreshold(50)
      
    def switch_Lanmark_mode(self):
        log.loginfo("Switch to Landmark mode")

        #step 1 config reflector
        self.config_reflector()
        self.nav350_setMode(Mode_landmark)

    def preset_for_mapping(self,layer):
        self.config_reflector()
        self.setMappingConfig(50,1,0,0,0)#mean:50% negative x:0 y:0 phi:0
        self.nav350_setCurrentLayer(layer)
        

    def switch_mapping_mode(self,layer):
        log.loginfo("Switch to Mapping mode")

        #mapping in layer
        self.preset_for_mapping(layer)
        self.nav350_setMode(Mode_mapping)
    def switch_navigating_mode(self,layer):
        log.loginfo("Switch to Navigating mode")

        self.nav350_setCurrentLayer(layer)
        self.setSlidingMean(5)
        self.nav350_setMode(Mode_navigation)

    def switch_mode_callback(self, msg):
        self.mode_change = msg.data
    ...
    def test_spin(self):
        ...
        if (self.result_socket == 1):
            # log.loginfo("Connect to Nav350 OK")
            if(self.mode_change == 1):
                if(self.mode_change != self.mode_change_old):
                    self.switch_Lanmark_mode()
                    self.mode_change_old = self.mode_change
                self.get_landmark_in_landmarkmode()
                # self.switch_Lanmark_mode()
            elif(self.mode_change == 2):
                if(self.mode_change != self.mode_change_old):
                    # self.preset_for_mapping(1)
                    self.nav350_setUserLevel()

                    self.preset_for_mapping(current_layer)
                    self.switch_mapping_mode(current_layer)
                    self.setDoMapping()
                    self.mode_change_old = self.mode_change
                self.setDoMapping()
                # self.get_all_landmark_in_layer(current_layer)
            ...
            elif(self.mode_change == 3):
                if(self.mode_change != self.mode_change_old):
                    self.nav350_setUserLevel()
                    self.switch_navigating_mode(current_layer  )
                    self.mode_change_old = self.mode_change
                self.get_all_landmark_in_layer(current_layer)
                # print(self.all_reflector)
                # for i in self.all_reflector:
    ...
    # def set_user_lever(self,level):
    #     i = 0
    #     while i < 11:
    #         try:
    #             # refsize = hex(refsize)
    #             # log.loginfo('[nav350] convert refsize:', str(refsize))
    #             self.data_setUser = self.s_socketSend(Code_start + Code_write + Code_space + Cmd_setUserLevel + Code_space + str(level) +Code_space + "F4724744"+Code_stop)
    #             if (self.data_setUser != None and self.socket_error_flag == 0):
    #                 self.data_setUser = self.data_setUser.split()
    ...
    #                 return self.data_setUser
    #             else:
    #                 i += 1
    #             if (i == 10):
    #                 log.loginfo('[NAV350] set Userlevel fail')
    #                 self.publish_error(enum.Nav350_Error.SetMode_Fail.value)
    #                 i = 0
    #         except Exception as exx:
    #             i += 1
    #             if (i == 10):
    #                 log.loginfo('[NAV350] set User fail: %s', exx)
    #                 self.publish_error(enum.Nav350_Error.SetMode_Fail.value)
    #                 return 0
    #     return 0
    def spin(self):
        ...
        while i <11:
            try: 
                ...
                if (i == 10):
                    ...
                    self.publish_error(enum.Nav350_Error.SetMode_Fail.value)
                    return 1
    def get_landmark_in_landmarkmode(self):
        i = 0
        while i <11:
            try: 
                self.get_pose_reflector = self.s_socketSend(Code_start + Code_method + Code_space + Cmd_get_scan_data + Code_space + '0' +Code_space + '0' + Code_stop)
                if (self.get_pose_reflector != None and self.socket_error_flag == 0):
                    self.get_pose_reflector = self.get_pose_reflector.split()
                    print("ID: ",self.get_pose_reflector[8],"X: ",self.convert_pos_data(self.get_pose_reflector[10])/1000,"Y: ", self.convert_pos_data(self.get_pose_reflector[11])/1000)
                # print(self.convert_pos_data(self.get_pose_reflector[5])/1000)
                # print(self.convert_pos_data(self.get_pose_reflector[6])/1000)
                if (i == 10):
                    log.loginfo('[NAV350] get landmark fail')
                    self.publish_error(enum.Nav350_Error.SetMode_Fail.value)
                    i = 0
                return 1
            except Exception as exx:
    ...
    def setRefType(self, reftype):
        ...
        while i < 11:
            try:
                # refsize = hex(refsize)
                # log.loginfo('[nav350] convert refsize:', str(refsize))
                self.data_setReftype = self.s_socketSend(Code_start + Code_write + Code_space + Cmd_Reftype + Code_space + str(reftype) + Code_stop)
                if (self.data_setReftype != None and self.socket_error_flag == 0):
                    self.data_setReftype = self.data_setReftype.split()
                    # log.loginfo('[NAV350] set Ref result: %s', self.data_setRefsize)
                    # log.loginfo('[nav350] sau khi convert:', self.convert_pos_data(self.data_setRefsize[2]))
                # if self.data_getRefsize[2] == '0':
                    return self.data_setReftype
                else:
                    i += 1
                if (i == 10):
                    log.loginfo('[NAV350] set Reftype fail')
                    self.publish_error(enum.Nav350_Error.SetMode_Fail.value)
                    i = 0
            except Exception as exx:
                i += 1
                if (i == 10):
                    log.loginfo('[NAV350] set Reftype fail: %s', exx)
                    self.publish_error(enum.Nav350_Error.SetMode_Fail.value)
                    return 0
        return 0
    
    def setRefNClosest(self, numble):
        i = 0
        while i < 11:
            try:
                # refsize = hex(refsize)
                # log.loginfo('[nav350] convert refsize:', str(refsize))
                self.data_setRefncloset = self.s_socketSend(Code_start + Code_write + Code_space + Cmd_Reftype + Code_space + str(numble) + Code_stop)
                if (self.data_setRefncloset  != None and self.socket_error_flag == 0):
                    self.data_setRefncloset  = self.data_setRefncloset .split()
                    # log.loginfo('[NAV350] set Ref result: %s', self.data_setRefsize)
                    # log.loginfo('[nav350] sau khi convert:', self.convert_pos_data(self.data_setRefsize[2]))
                # if self.data_getRefsize[2] == '0':
                    return self.data_setRefncloset 
                else:
                    i += 1
                if (i == 10):
                    log.loginfo('[NAV350] set Ref fail')
                ...
                if (i == 10):
                    ...
                    return 0
        return 0
    def setRefThreshold(self, threshold):
        i = 0
        while i < 11:
            try:
                # refsize = hex(refsize)
                # log.loginfo('[nav350] convert refsize:', str(refsize))
                self.data_setRefThreshold= self.s_socketSend(Code_start + Code_write + Code_space + Cmd_Reflector_threshold + Code_space + str(threshold) + Code_stop)
                if (self.data_setRefThreshold  != None and self.socket_error_flag == 0):
                    self.data_setRefThreshold  = self.data_setRefThreshold.split()
                    # log.loginfo('[NAV350] set Ref result: %s', self.data_setRefsize)
                    # log.loginfo('[nav350] sau khi convert:', self.convert_pos_data(self.data_setRefsize[2]))
                # if self.data_getRefsize[2] == '0':
                    return self.data_setRefThreshold
                else:
                    i += 1
                if (i == 10):
                    log.loginfo('[NAV350] set Ref fail')
                ...
                if (i == 10):
                    ...
                    return 0
        return 0
    def setDoMapping(self):
        i = 0
        while i < 11:
            try:
                # refsize = hex(refsize)
                # log.loginfo('[nav350] convert refsize:', str(refsize))
                self.data_setDoMapping= self.s_socketSend(Code_start + Code_method + Code_space + Cmd_doMapping+ Code_stop)
                if (self.data_setDoMapping  != None and self.socket_error_flag == 0):
                    self.data_setDoMapping  = self.data_setDoMapping.split()
                    print(self.data_setDoMapping)
                    # log.loginfo('[NAV350] set Ref result: %s', self.data_setRefsize)
                    # log.loginfo('[nav350] sau khi convert:', self.convert_pos_data(self.data_setRefsize[2]))
                # if self.data_getRefsize[2] == '0':
                    return 1
                ...
                if (i == 10):
                    ...
                    return 0
        return 0
    def setMappingConfig(self, mean,negative,x,y,phi):
        i = 0
        while i < 11:
            try:
                # refsize = hex(refsize)
                # log.loginfo('[nav350] convert refsize:', str(refsize))
                self.data_setMappingConfig= self.s_socketSend(Code_start + Code_write + Code_space + Cmd_ConfigMapping + Code_space + str(mean) +Code_space + str(negative)+Code_space+str(x)+Code_space+str(y)+Code_space+str(phi)+ Code_stop)
                if (self.data_setMappingConfig  != None and self.socket_error_flag == 0):
                    self.data_setMappingConfig  = self.data_setMappingConfig.split()
                    # log.loginfo('[NAV350] set Ref result: %s', self.data_setRefsize)
                    # log.loginfo('[nav350] sau khi convert:', self.convert_pos_data(self.data_setRefsize[2]))
                # if self.data_getRefsize[2] == '0':
                    return self.data_setMappingConfig
                else:
                    i += 1
                if (i == 10):
                    log.loginfo('[NAV350] set Ref fail')
